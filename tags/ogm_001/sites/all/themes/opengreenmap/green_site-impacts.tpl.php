...
function output_impacts($node)
{
	...
	$add_impact .= views_embed_view('impacts', 'default', $node->nid);
	$add_impact .= '</div>';
	...
	if ($line->cnt > 0) {
      ...
      t('@count discovered this site because of this map. <a href="@add_impact">Did you?</a>', array(
            '@count' => format_plural($line->cnt, '1 person', '@count people'),
            '@add_impact' => base_path().'node/add/impact?theme=simple&destination=node/'.$node->nid.'/simple&nid='.$node->nid.'&node_title='.$node->title)).
    '</p>';
  }
	$add_impact .= '</div>';
