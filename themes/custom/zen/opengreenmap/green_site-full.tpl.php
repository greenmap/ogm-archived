...
if($node->field_involved[0]['value'] == 'yes') { 
  ...
  $comments = '<p>' . comment_render($node) . '</p>';
	...
	$multimedia .= '<div id="multimedia_main">';
		if (empty($media))
			$multimedia .= '<div id="multimedia_main_na">Be the first to add a <a href="'.base_path().'node/add/photo?theme=simple&destination=node/'.$node->nid.'/simple&nid='.$node->nid.'&node_title='.$node->title.'">photo</a> or <a href="'.base_path().'node/add/video?theme=simple&destination=node/'.$node->nid.'/simple&nid='.$node->nid.'&node_title='.htmlspecialchars($node->title).'">video</a>!</div>';
		else
			// display first video
