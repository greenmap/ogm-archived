...
$(document).ready(function(){
  log( 'Document Ready' );
  // disable the pdf field - not doing this in PHP in case JS is disabled on users computer.
  ...
  $('#edit-field-map-in-greenhouse-0-url').change(function(){
    // don't want to let people put any URL in here - danger of cross site
    // scripting, so ideally should just take last arg and append to
    // hard-coded url
    var nodeid = $('#edit-field-map-in-greenhouse-0-url').attr("value");
    log( 'nodeid entered:', nodeid );
...
function jsonGM( json ) {
  ...
  if(json.type == 'content_map'){
    ...
    $('.link-field-url > .form-item > .description').text(Drupal.t('Success: this is a valid map ID'));
    // $(this).attr('value','');
  } else {
    ...
    $('.link-field-url > .form-item > .description').text(Drupal.t('Error: this is not a map ID'));
  }
}
