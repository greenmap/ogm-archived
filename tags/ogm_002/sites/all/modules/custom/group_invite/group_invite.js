		...
		http_request.onreadystatechange = function() {
			...
			if (http_request.status == 200) {// success
				if(http_request.responseText == ''){
					...
					// new email
					document.getElementById("inDB"+num).value = '0';
					document.getElementById("DIVemail"+num).innerHTML = Drupal.t("New member");
				} else if(http_request.responseText == "ERROR"){
					// email doesn't meet our requirements
					document.getElementById("inDB"+num).value = '0';
					document.getElementById("DIVemail"+num).innerHTML = Drupal.t("Email address is not valid");
				} else if(http_request.responseText == "ERROR2"){
					// user tries to invite him/her self
					document.getElementById("inDB"+num).value = '0';
					document.getElementById("DIVemail"+num).innerHTML = Drupal.t("It is forbidden to change your own roles");
				} else {
					// in the DB
					document.getElementById("inDB"+num).value = '1';
				//	alert(document.getElementById("inDB"+num).value);
					document.getElementById("DIVemail"+num).innerHTML = Drupal.t("Old member of the Open Green Map");
					...
					role_request.onreadystatechange = function(){
						...
						if (role_request.status == 200) {// success
							...
							alert(Drupal.t('problems with ajax query'));
						}
					};
				}
			} else {// failed
				document.getElementById("DIVemail"+num).innerHTML = Drupal.t("Email checking failed");
			}
	};
}
