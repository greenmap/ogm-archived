...
function keyIcon_OnMouseOver(e) { 
  ...
  container.innerHTML = '<h4 class="icon_description_tooltip_title">'
    + $(this).find('img').attr('alt') + '</h4><p class="icon_description_tooltip_full">'
    + $(this).find('img').attr('rel') + '</p>';
...
function keyTitle_OnMouseOver(e) { 
  // create a div and append it to the body
  var container = document.createElement('div');
  container.innerHTML = '<p class="icon_description_tooltip_title">'
    + $(this).find('span').attr('rel') + '</p>';
  container.setAttribute('class', 'icon_description_tooltip');
  container.style.display = 'none';
  container.style.top = e.pageY + 'px';
  document.getElementsByTagName('body')[0].appendChild(container);
  ...
  // display the div
  $('.icon_description_tooltip').fadeIn(2000);
}

function keyTitle_OnMouseOut() {
  // remove any div from the DOM tree
  jQuery.each($('.icon_description_tooltip'), function() {
    document.getElementsByTagName('body')[0].removeChild(this);
  });
...
$(document).ready(function() {
  ...
  $('.key_title').hoverIntent(keyTitle_OnMouseOver,keyTitle_OnMouseOut);
  ...
  jQuery.each($('.key_title a span'), function() {
    // hide the title tooltip as we're replacing it with this custom div
    var description = $(this).attr('title');
    $(this).attr('rel', description);
    $(this).attr('title', '');
